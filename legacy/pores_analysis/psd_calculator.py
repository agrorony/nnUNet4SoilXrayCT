...
try:
    ...
    from .psd_diagnostics import DiagnosticsCollector
except ImportError:
...
def compute_psd_from_opening_map(
    ...
    Args:
        ...
        diagnostics: Optional diagnostics collector to capture observability data
    ...
    Returns:
        Dictionary with:
            ...
            - 'diagnostics': Diagnostics payload (if enabled)
            - 'voxel_spacing': Physical voxel spacing used for binning
    ...
    if diagnostics is not None:
        diagnostics.log_raw_diameters(pore_diameters)
    ...
    if diagnostics is not None:
        diagnostics.log_binning(volume_counts, bin_edges_px, bin_edges_um)
    ...
    if diagnostics is not None:
        diagnostics.log_post_psd(differential_volume, bin_centers_um)
...
def compute_psd(
    ...
    Args:
        ...
        diagnostics: Optional diagnostics configuration (histogram bins, output).
    ...
    diag_collector = DiagnosticsCollector(diagnostics, default_tag=run_id or "psd")
    diag_collector.record_metadata(
        volume_shape=binary_volume.shape,
        use_gpu=use_gpu,
        use_chunking=use_chunking,
        voxel_spacing=physical_voxel_spacing,
    )
    ...
    diag_collector.log_edt_map(edt_map, voxel_spacing=physical_voxel_spacing)
    ...
    diag_collector.log_opening_map(opening_map)
    ...
    if len(psd['bin_centers_um']) > 0:
        print(f"Diameter range: {psd['bin_centers_um'].min():.2f} - {psd['bin_centers_um'].max():.2f} um")
    else:
...
if __name__ == "__main__":
    ...
    print(f"Expected peaks: ~{2*radius1} um and ~{2*radius2} um")
    ...
    if len(psd['bin_centers_um']) > 0:
        ...
        print(f"  Diameter range: {psd['bin_centers_um'].min():.2f} - {psd['bin_centers_um'].max():.2f} um")
        print(f"  Peak at: {psd['bin_centers_um'][psd['volume_counts'].argmax()]:.2f} um")
    else:
