"""
Fine-tune from fresh_bnei_reem_i3_lowlr checkpoint on fine_tuning_annotations.nii.gz.
GPU 1  |  LR 2e-3 (low)  |  Early stopping
Output: multi_sample_fresh_bnei_reem_i4
"""
import sys
...
NEW_ANNOTATION_PATH = os.path.join(HIVE_BASE, 'fresh_train_annotations_bnei_reem', 'fine_tuning_annotations.nii.gz')
# Original LR: 1e-2  |  New LR: 2e-3  (5x reduction for fine-tuning stability)
TRAINER_NAME        = 'nnUNetTrainer_betterIgnoreSampling_earlyStopValLoss_lowlr'
LOCAL_BASE          = os.path.join(HIVE_BASE, 'multi_sample_fresh_bnei_reem_i4')
...
# Pretrained weights: i3_lowlr checkpoint_final
I3_LOWLR_BASE = os.path.join(HIVE_BASE, 'multi_sample_fresh_bnei_reem_i3_lowlr')
PRETRAINED_CHECKPOINT = os.path.join(
    I3_LOWLR_BASE, 'nnUNet_results', 'Dataset777_GCEF',
...
assert os.path.isfile(PRETRAINED_CHECKPOINT), f'i3_lowlr checkpoint not found: {PRETRAINED_CHECKPOINT}'
...
if not log_files:
    ...
    plt.title(f'Training Metrics — fresh_bnei_reem_i4  (LR 2e-3, fine-tune from i3_lowlr)\n{len(ep)} epochs', fontweight='bold')
...
print('\n=== fresh_bnei_reem_i4 training complete ===')
