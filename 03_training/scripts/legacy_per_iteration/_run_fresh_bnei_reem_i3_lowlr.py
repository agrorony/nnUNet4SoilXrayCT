...
for mod_name in list(sys.modules):
    if 'nnUNetTrainer_betterIgnoreSampling' in mod_name:
...
print('[OK] Custom trainer registered:', nnUNetTrainer_betterIgnoreSampling_earlyStopValLoss_lowlr.__name__)
print(f'     initial_lr = {nnUNetTrainer_betterIgnoreSampling_earlyStopValLoss_lowlr.__init__.__qualname__}')
...
# Same annotations as i3
NEW_ANNOTATION_PATH = os.path.join(
    HIVE_BASE, 'fresh_train_annotations_bnei_reem', 'annotations_i3.nii.gz'
)
...
# Original LR: 1e-2  |  New LR: 2e-3  (5x reduction)
TRAINER_NAME = 'nnUNetTrainer_betterIgnoreSampling_earlyStopValLoss_lowlr'

LOCAL_BASE          = os.path.join(HIVE_BASE, 'multi_sample_fresh_bnei_reem_i3_lowlr')
...
# Pretrained weights: fresh_bnei_reem_i2 checkpoint (same as i3)
I2_BASE = os.path.join(HIVE_BASE, 'multi_sample_fresh_bnei_reem_i2')
PRETRAINED_CHECKPOINT = os.path.join(
    I2_BASE, 'nnUNet_results', 'Dataset777_GCEF',
...
assert os.path.isfile(PRETRAINED_CHECKPOINT), f'i2 checkpoint not found: {PRETRAINED_CHECKPOINT}'
print(f'Pretrained weights (i2): {PRETRAINED_CHECKPOINT}')
...
img_arr = tifffile.imread(RAW_TIFF_PATH)
...
print('Early stopping env:', {
    k: os.environ[k] for k in (
        'NNUNET_EARLY_STOP_ENABLED', 'NNUNET_EARLY_STOP_PATIENCE',
        'NNUNET_EARLY_STOP_MIN_DELTA'
    )
})
print(f'Trainer: {TRAINER_NAME}  (LR=2e-3, was 1e-2)')
...
# Clean explicit line logged by the new trainer class every epoch
mean_dice_re  = re.compile(r'mean_dice_computed\s+(-?[\d.eE]+)')
            ...
            if m:
                ...
                if current_epoch not in epoch_data:
                    epoch_data[current_epoch] = {
                        'epoch': current_epoch, 'fold': fold_name,
...
# Verify every row has Dice
missing_dice = summary['mean_dice'].isna().sum()
if missing_dice > 0:
    print(f'WARNING: {missing_dice} epochs missing mean_dice in CSV')
else:
    print(f'[OK] All {len(summary)} epochs have mean_dice populated')
...
plt.title(
    'Training Metrics -- fresh_bnei_reem_i3_lowlr\n'
    '(fine-tune from i2, LR=2e-3 vs i3 LR=1e-2)',
...
# ── Inference ─────────────────────────────────────────────────────────────────
print('\n' + '=' * 70)
print('Running inference with fresh_bnei_reem_i3_lowlr model ...')
print('=' * 70)

inf_script = os.path.join(REPO_DIR, '_run_inference_fresh_bnei_reem_i3_lowlr.py')
...
print('\n[OK] fresh_bnei_reem_i3_lowlr pipeline complete.')
