"""
Train from scratch on mishmar_hanegev with new nii.gz annotation.
GPU 1  |  LR 1e-2 (regular)  |  Early stopping
Output: multi_sample_mishmar_hnegev_scratch
...
time.sleep(7200)
print('Starting mishmar_hnegev_scratch training.', flush=True)
...
LOCAL_BASE          = os.path.join(HIVE_BASE, 'multi_sample_mishmar_hnegev_scratch')
...
print(f'Trainer:     {TRAINER_NAME}  (LR=1e-2)')
...
print(f'Trainer: {TRAINER_NAME}  (LR=1e-2, no pretrained weights)')
...
if not log_files:
    ...
    plt.title(f'Training Metrics — mishmar_hnegev_scratch  (LR 1e-2, from scratch)\n{len(ep)} epochs', fontweight='bold')
...
print('\n=== mishmar_hnegev_scratch training complete ===')
