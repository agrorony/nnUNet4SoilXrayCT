"""
Standalone inference for fresh_bnei_reem model on nlm_volume.

Reuses pre-split chunks from bnei_reem_iter04/inference_input (identical plans).
Outputs to bnei_reem_fresh_bnei_reem/inference_concatenated/nlm_volume.nii.gz.
"""
import os
import gc
import sys
...
if __name__ == '__main__':
    ...
    MODEL_DIR = os.path.join(
        HIVE_BASE, 'multi_sample_fresh_bnei_reem', 'nnUNet_results', 'Dataset777_GCEF',
        f'{TRAINER_NAME}__nnUNetPlans__3d_fullres'
    )
    SPLIT_DIR   = os.path.join(HIVE_BASE, 'bnei_reem_iter04', 'inference_input')
    SAMPLE_BASE = os.path.join(HIVE_BASE, 'bnei_reem_fresh_bnei_reem')
    ...
    print(f'\n{"="*70}')
    print(f'DONE - fresh_bnei_reem prediction:')
