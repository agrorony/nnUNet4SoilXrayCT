"""POM interface metrics -- POM-pore, POM-matrix, specific surface area (SSA),
interfacial area density (IAD). Track B, step 2(a) of the POM deep-dive.

Per pom_interface_metrics_prompt.md. Literature basis: Vogel & Roth (2001)
for surface area as a Minkowski functional; Houston et al. (2013) /
Juyal et al. (2021) for the solid-pore interfacial area method (POM-adjacent
precedent); Schlueter et al. (2014) for the voxel-face-count vs
marching-cubes cross-check; Herring et al. (2015) for the per-bulk-volume
density normalization convention (already used for connectivity density in
this pipeline).

Scope: the same 3 POM-valid volumes used throughout this project's POM work
(Bnei Re'em canonical, Mishmar native 5.85um, Mishmar 2nd sample 8.8um
label-downsampled ~15um) -- NOT the 2026-08-29/30 ROI-expanded Mishmar crops.
Reuses each volume's own pinned A1 noise-floor cutoff from its own prior
summary_pom_metrics.json rather than re-deriving it.

Part 1 (voxel-face contact fractions) intentionally duplicates the exact
surface/dilation logic from
pom_analysis_20260815_light/scripts/run_pom_analysis.py so the Bnei Re'em
pom_pore_contact_fraction reproduces 0.6422932338325659 bit-for-bit -- this
is a sanity anchor, not a new methodology.

Part 2 (marching-cubes surface + phase split) reuses the same
marching_cubes(level=0.5, spacing=(voxel_um,)*3) call used for sphericity in
pom_analysis_20260826_final_shape/scripts/run_pom_shape_clustering_final.py.
"""
...
OUT_ROOT = SCRIPT_DIR.parent  # .../pom_analysis_20260830_interface_metrics
...
SOILS = {
    ...
    "mishmar_native": dict(
        soil_label="Mishmar HaNegev (Loess) -- native 5.85um",
        path=r"\\hive3065\Yael_Mishael\Rony\remote_computer backup\nnUNet_resources\mishmar_hanegev_maoz_3_5p85um\inference_output_concat_loess_i2\mishmar_hanegev_maoz_3_5p85um.nii.gz",
        voxel_um=5.85,
        pore_label=5,
        pom_label=2,
        cutoff_voxels=21,  # pinned in pom_analysis_20260815_light/mishmar/summary_pom_metrics.json
        reference_pore_contact_fraction=0.5623384431086234,
...
def process_soil(soil_key: str, cfg: dict) -> Dict:
    ...
    if cfg.get("check_reproduction"):
        ref = cfg["reference_pore_contact_fraction"]
        diff = abs(pom_pore_contact_fraction - ref)
        reproduction_check = {
            "reference_value": ref,
            "computed_value": pom_pore_contact_fraction,
            "abs_diff": diff,
            "matched": bool(diff < 1e-9),
        }
        log(f"  REPRODUCTION CHECK vs {ref}: diff={diff:.3e} matched={reproduction_check['matched']}")
        if not reproduction_check["matched"]:
