"""Part 3/4 -- generalized POM shape/spatial clustering, parameterized by
dataset paths and resolvability cutoff, for the ROI-expansion prompt.

This is the pinned methodology from pom_analysis_20260826_final_shape/
scripts/run_pom_shape_clustering_final.py (marching-cubes sphericity,
eigenvalue-floored PCA elongation/flatness, per-run-only StandardScaler,
KMeans + silhouette per-volume clustering, cross-volume archetype matching
on centroids) -- UNCHANGED algorithmically. The only differences from that
script:
  1. DATASETS is loaded from a --datasets-json file instead of being
     hardcoded, so Part 3 can point at whichever crop each volume ended up
     with (enlarged-and-passed / existing-because-insufficient-margin /
     existing-because-failed-sanity).
  2. MIN_VOXELS_ACROSS_DIAMETER is a --cutoff-voxels CLI argument instead
     of a hardcoded constant, so Part 4's 15/20/25-voxel sensitivity sweep
     can reuse this same script instead of a hand-maintained fork.

Usage:
    python run_pom_shape_clustering_generalized.py --run-name roi20 \
        --datasets-json datasets_this_run.json --cutoff-voxels 20
"""
...
def compute_soil_features(soil_key: str, cfg: dict, min_voxels_across_diameter: float) -> pd.DataFrame:
...
def main(run_name: str, datasets: Dict[str, dict], min_voxels_across_diameter: float,
         do_plots: bool = True) -> None:
    soil_keys = list(datasets.keys())
    log(f"=== Run '{run_name}': {soil_keys}  cutoff={min_voxels_across_diameter} vox ===")
    log(f"Groups: { {sk: SOIL_GROUP[sk] for sk in soil_keys} }")

    dfs = {}
    cutoff_report = {}
    for sk in soil_keys:
        dfs[sk] = compute_soil_features(sk, datasets[sk], min_voxels_across_diameter)
        cutoff_report[sk] = {
    ...
    log("Fitting StandardScaler across ONLY this run's volumes -- no pooling with any other branch/soil...")
    ...
    mishmar_keys = [sk for sk in soil_keys if SOIL_GROUP.get(sk) == "mishmar"]
    bnei_keys = [sk for sk in soil_keys if SOIL_GROUP.get(sk) == "bnei_reem"]
    mishmar_matrix = np.stack([proportion_vectors[sk] for sk in mishmar_keys]) if mishmar_keys else None
    mishmar_mean = mishmar_matrix.mean(axis=0) if mishmar_matrix is not None else None
    mishmar_se = (mishmar_matrix.std(axis=0, ddof=1) / np.sqrt(len(mishmar_keys))
                  if mishmar_matrix is not None and len(mishmar_keys) > 1 else None)
    bnei_reem_vec = np.array(proportion_vectors[bnei_keys[0]]) if bnei_keys else None
    diff_from_mishmar_mean = (bnei_reem_vec - mishmar_mean).tolist() if (bnei_reem_vec is not None and mishmar_mean is not None) else None
    se_multiples = (
        np.where(mishmar_se > 0, (bnei_reem_vec - mishmar_mean) / mishmar_se, np.nan).tolist()
        if (bnei_reem_vec is not None and mishmar_se is not None) else None
    )

    proportion_report = {
        "n_archetypes": n_archetypes,
        "archetype_silhouette_on_centroids": float(best_ascore) if best_ascore == best_ascore else None,
        "min_voxels_across_diameter": min_voxels_across_diameter,
        "note": "n=1 on the Bnei Re'em side -- this is a descriptive comparison, not an inferential test.",
        "per_volume_vectors": {sk: {"vector": proportion_vectors[sk], "n_objects": int(len(dfs[sk]))} for sk in soil_keys},
        "mishmar_mean": mishmar_mean.tolist() if mishmar_mean is not None else None,
        "mishmar_se": mishmar_se.tolist() if mishmar_se is not None else None,
        "bnei_reem_minus_mishmar_mean": diff_from_mishmar_mean,
        "bnei_reem_deviation_in_mishmar_se_units": se_multiples,
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--datasets-json", required=True, help="JSON file: {soil_key: {label,path,voxel_um,pore_label,pom_label}}")
    parser.add_argument("--cutoff-voxels", type=float, default=20.0)
    parser.add_argument("--no-plots", action="store_true")
    args = parser.parse_args()
    with open(args.datasets_json, encoding="utf-8") as fh:
        datasets = json.load(fh)
    main(args.run_name, datasets, args.cutoff_voxels, do_plots=not args.no_plots)
