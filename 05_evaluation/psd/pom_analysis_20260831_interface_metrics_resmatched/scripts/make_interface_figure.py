...
with open(BASE / "all_soils_interface_summary.json") as f:
    D = json.load(f)
...
LABEL_L = "Loess (Mishmar HaNegev, n=2, resolution-matched ~15um)"
...
note = (
    "RESOLUTION-MATCHED RE-RUN (2026-08-31): both Mishmar\n"
    "replicates now label-downsampled to ~15um (native 5.85um\n"
    "sample + 8.8um second specimen). The 2026-08-30 run mixed\n"
    "a 5.85um replicate with a ~15um one -- see final_report.md's\n"
    "'Resolution correction' section for the before/after table.\n\n"
    "Bnei Re'em: single volume (n=1), no error bar --\n"
    "descriptive point, not a distribution.\n"
    "Mishmar: mean +/- SE across 2 resolution-matched\n"
    "reconstructions (n=2).\n\n"
    "No significance testing shown: n=1 for Bnei Re'em\n"
    "precludes an omnibus test.\n\n"
    ...
    "pore-vs-matrix SPLIT agrees within ~8pp (panels a-c\n"
    "rely on the split, not the absolute totals)."
)
ax.text(0.0, 0.62, note, transform=ax.transAxes, fontsize=7.6, va="top",
        bbox=dict(boxstyle="round", fc="white", ec="0.6", lw=0.7))

fig.suptitle("POM interface metrics (resolution-matched) - Bnei Re'em (Vertisol) vs. Mishmar HaNegev (Loess)",
             fontsize=12, y=0.995)
fig.tight_layout(rect=[0, 0, 1, 0.97])
fig.subplots_adjust(hspace=0.55, wspace=0.32)

out_png = BASE / "pom_interface_metrics_figure.png"
out_svg = BASE / "pom_interface_metrics_figure.svg"
