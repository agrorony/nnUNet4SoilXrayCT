...
def _build_compute_kwargs(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if diagnostics_config:
        diag_cfg = dict(diagnostics_config)
        diag_cfg.setdefault("output_dir", paths.get("output_dir", "."))
        diag_cfg.setdefault("run_tag", diag_cfg.get("run_tag") or run_id_str or "psd")
        kwargs["diagnostics"] = diag_cfg
