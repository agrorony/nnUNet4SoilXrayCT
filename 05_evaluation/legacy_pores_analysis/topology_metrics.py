"""
Topology & Connectivity Metrics Extension
==========================================

Extends the existing PSD pipeline (psd_calculator.py / local_thickness.py /
distance_transform.py) with connectivity-conditioned distance maps, Euler
...
`analysis/pore_metrics_research/decisions.md` (Stage 1). See that file for
full justification/citations and confidence flags (D1-D5).
...
diameter maps) or run alongside it.
"""
...
def distance_map(
    ...
    voxel of ``target_label``, optionally restricted to the connectivity-
    conditioned (percolating) subset per D1.

    Args:
        volume: 3D integer-labeled array (matrix/pore/POM).
        target_label: integer label of the phase to compute distances to
            (e.g. pore label for distance-to-pore, POM label for
            distance-to-POM).
        voxel_size_um: isotropic physical voxel edge length (um).
        connected_only: if True, use only the percolating subset of the
            target phase (D1) as the foreground for the distance transform;
            if False, use the full (unconditioned) target phase.
        axis: percolation axis used when connected_only=True (default Z).

    Returns:
        Float32 array, same shape as ``volume``, distance in um from every
        voxel to the nearest qualifying target voxel (0 at target voxels
        themselves).
    """
    target_mask = (volume == target_label)

    if connected_only:
        target_mask = get_percolating_mask(target_mask, axis=axis)

    if not np.any(target_mask):
        warnings.warn(
            f"No voxels found for target_label={target_label} "
            f"(connected_only={connected_only}); returning all-inf distance map.",
            UserWarning,
        )
        return np.full(volume.shape, np.inf, dtype=np.float32)
    ...
    # ~target_mask makes non-target voxels foreground, and their distance is
    # computed to the nearest target (True) voxel, which is what we want.
    dmap = distance_transform_edt(~target_mask, sampling=(voxel_size_um,) * 3)
...
def tortuosity_diffusive(
    ...
    try:
        ...
        warnings.warn(
            ...
            "Add 'porespy' to the environment (see environment.yml).",
            UserWarning,
        )
