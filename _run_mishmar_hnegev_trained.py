"""
Fine-tune from fresh_bnei_reem_i3_scratch checkpoint on mishmar_hanegev with new nii.gz annotation.
GPU 0  |  LR 2e-3 (low)  |  Early stopping
Output: multi_sample_mishmar_hnegev_trained
Starts after 2-hour delay.
"""
import sys
...
import time
print('Waiting 2 hours before starting...', flush=True)
time.sleep(7200)
print('Starting mishmar_hnegev_trained training.', flush=True)
...
SAMPLE_ID           = 'mishmar_hanegev_Cu011_samp_2_Rec_nlm'
RAW_TIFF_PATH       = os.path.join(HIVE_ROOT, '10.5', f'{SAMPLE_ID}.tif')
NEW_ANNOTATION_PATH = os.path.join(HIVE_BASE, 'mishmar_hanegev_new_work', 'annotations', f'{SAMPLE_ID}.nii.gz')
TRAINER_NAME        = 'nnUNetTrainer_betterIgnoreSampling_earlyStopValLoss_lowlr'
LOCAL_BASE          = os.path.join(HIVE_BASE, 'multi_sample_mishmar_hnegev_trained')
...
# Pretrained weights: fresh_bnei_reem_i3_scratch checkpoint_final
SCRATCH_BASE = os.path.join(HIVE_BASE, 'multi_sample_fresh_bnei_reem_i3_scratch')
PRETRAINED_CHECKPOINT = os.path.join(
    SCRATCH_BASE, 'nnUNet_results', 'Dataset777_GCEF',
...
if not os.path.isfile(PRETRAINED_CHECKPOINT):
    PRETRAINED_CHECKPOINT = PRETRAINED_CHECKPOINT.replace('checkpoint_final.pth', 'checkpoint_best.pth')
assert os.path.isfile(PRETRAINED_CHECKPOINT), f'Scratch checkpoint not found: {PRETRAINED_CHECKPOINT}'
...
print(f'Trainer:     {TRAINER_NAME}  (LR=2e-3)')
...
print('Loading annotation (nii.gz)...')
...
affine = np.eye(4)
nib.save(nib.Nifti1Image(img_crop, affine), os.path.join(images_dir, f'{SAMPLE_ID}_0000.nii.gz'))
del img_crop
lbl_nii = nib.Nifti1Image(ann_nnunet, affine)
lbl_nii.header.set_data_dtype(np.uint8)
nib.save(lbl_nii, os.path.join(labels_dir, f'{SAMPLE_ID}.nii.gz'))
del ann_nnunet
...
if not log_files:
    ...
    plt.title(f'Training Metrics — mishmar_hnegev_trained  (LR 2e-3, fine-tune from i3_scratch)\n{len(ep)} epochs', fontweight='bold')
...
print('\n=== mishmar_hnegev_trained training complete ===')
