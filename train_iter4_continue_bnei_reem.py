...
# Pin to GPU 0 — parallel fresh run gets GPU 1
...
LOCAL_BASE = os.path.join(HIVE_BASE, 'multi_sample_iter04_continue')
...
# --- iter04 checkpoint (continuation starting point) ---
ITER04_BASE = os.path.join(HIVE_BASE, 'multi_sample_iter04')
PRETRAINED_CHECKPOINT = os.path.join(
    ITER04_BASE, 'nnUNet_results', 'Dataset777_GCEF',
    f'{TRAINER_NAME}__nnUNetPlans__3d_fullres',
    'fold_0', 'checkpoint_final.pth'
)
assert os.path.isfile(PRETRAINED_CHECKPOINT), f'Iter04 checkpoint not found: {PRETRAINED_CHECKPOINT}'
...
plt.title('Training Metrics — iter04 continuation (bnei_reem, merged_GT)', fontsize=14, fontweight='bold')
plt.tight_layout()
...
print(f'\n[OK] train_iter4_continue_bnei_reem complete.')
